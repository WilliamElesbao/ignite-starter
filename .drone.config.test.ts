...
describe("Drone CI Configuration Validation", () => {
  describe("Property 1: Drone Pipeline Step Dependencies", () => {
    ...
    it("should have all check steps depend on install step", () => {
      ...
      const checkSteps = ["typecheck", "lint"];
    ...
    it("should have exactly 3 steps: install, typecheck, lint", () => {
      const config = parseDroneConfig();
      const expectedSteps = ["install", "typecheck", "lint"];
  ...
  describe("Additional Configuration Validation", () => {
    ...
    it("should have install step with lockfile check and bun install", () => {
      const config = parseDroneConfig();
      const installStep = config.steps.find((s) => s.name === "install");

      expect(installStep?.commands).toBeDefined();
      expect(installStep?.commands).toContain("test -f bun.lock");
      expect(installStep?.commands).toContain("bun install");
    });
    ...
    it("should have lint step with scoped biome ci command", () => {
      const config = parseDroneConfig();
      const lintStep = config.steps.find((s) => s.name === "lint");

      expect(lintStep?.commands).toBeDefined();
      expect(
        lintStep?.commands?.[0]?.includes("bun biome ci apps/web apps/backend"),
      ).toBe(true);
    });
  });
});
